...
import { Copy, Check, Image as ImageIcon, Calendar, MessageSquare, ExternalLink } from 'lucide-react';
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mb-20">
      ...
      <div className="flex justify-center gap-4 mb-10">
        ...
        <button onClick={() => setMode('booking')} className={`px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${mode === 'booking' ? 'bg-blue-600 text-white shadow-lg' : 'bg-white/5 text-gray-500 border border-white/5'}`}><Calendar size={14} className="inline mr-2"/> {t.bookingBtn}</button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 text-left">
        <div className="lg:col-span-2 bg-[#14151a] p-8 md:p-12 rounded-[2.5rem] border border-white/5 shadow-2xl space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-6">
              <div>
                ...
                <input type="text" placeholder="01121281024" value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full bg-black/50 border border-white/10 rounded-2xl p-5 text-white outline-none focus:border-yellow-500 transition-all font-bold" />
              </div>
              <div>
          ...
          <div className="bg-black/30 p-6 rounded-2xl border border-dashed border-white/10">
            ...
            <div className="flex gap-4">
              <button onClick={copyToClipboard} disabled={!fullLink} className="flex-1 flex items-center justify-center gap-2 bg-white text-black py-4 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-yellow-500 transition-all disabled:opacity-20">{copied ? <Check size={14} /> : <Copy size={14} />} {t.copyBtn}</button>
              <a href={fullLink} target="_blank" className={`flex-1 flex items-center justify-center gap-2 bg-yellow-500 text-black py-4 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-white transition-all ${!fullLink && 'pointer-events-none opacity-20'}`}>{t.testBtn} <ExternalLink size={14}/></a>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#14151a] p-8 rounded-[2.5rem] border border-white/5 shadow-2xl flex flex-col items-center justify-center">
          ...
          <button onClick={downloadQRCode} disabled={!fullLink} className="w-full flex items-center justify-center gap-2 bg-yellow-500 text-black py-4 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-white transition-all disabled:opacity-10 shadow-lg">{t.downloadBtn}</button>
        </div>
      </div>
    </div>
  );
