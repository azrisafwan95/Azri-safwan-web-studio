...
` + body.slice(blockEnd)}<\/script>`;
	...
	function toRelativeImport(fromFile, targetFile) {
		const relative = pathe.relative(pathe.dirname(fromFile), targetFile).replace(/\\/g, "/");
		return relative.startsWith(".") ? relative : `./${relative}`;
	}
	function patchSvelteKitStaticHtml() {
		const buildDir = pathe.resolve(viteConfig.root, "build");
		let initFile = emittedFileName ? pathe.resolve(buildDir, emittedFileName) : void 0;
		if (!initFile || !fs.existsSync(initFile)) initFile = walkFiles(buildDir, (fileName) => fileName.endsWith(".js")).find((file) => {
...
const processModuleAssets = (bundle, filesMap, moduleMatcher, options = {}) => {
	...
	for (const [fileName, fileData] of Object.entries(bundle)) {
		...
		for (const modulePath of Object.keys(fileData.modules)) {
			const comparableModulePath = options.root ? pathe.default.resolve(options.root, modulePath) : modulePath;
			const comparableModulePaths = [comparableModulePath];
			if (options.stripKnownJsExtensions) {
				const ext = pathe.default.extname(comparableModulePath);
				if (JS_EXTENSIONS.includes(ext)) comparableModulePaths.push(pathe.default.join(pathe.default.dirname(comparableModulePath), pathe.default.basename(comparableModulePath, ext)));
			}
			...
			if (!disableAssetsAnalyze) {
				...
				processModuleAssets(bundle, filesMap, (modulePath) => {
					return exposesModules.find((exposeModule) => {
						return modulePath === pathe.resolve(root, exposeModule);
					});
				}, {
					root,
		...
		generateBundle(_, bundle) {
			...
			processModuleAssets(bundle, filesMap, (modulePath) => {
				return exposeEntries.find(([_, exposeOptions]) => {
					return modulePath === pathe.resolve(root, exposeOptions.import);
				})?.[1].import;
			}, {
				root,
				stripKnownJsExtensions: true
...
function isFederationHtmlPreloadDependency(dep, includeSharedRuntime = false) {
	const file = pathe.default.basename(dep);
