...
const RESOLVE_DELAY = 25, FLUSH_DELAY = 200, isDataTransfer = (value) => value?.constructor.name === "DataTransfer";
...
const RestoreDOM = IS_ANDROID ? RestoreDOMComponent : ({
  ...
  const androidInputManagerRef = useRef(void 0), onDOMSelectionChange = useMemo(() => throttle(() => {
    ...
    if ((IS_ANDROID || !editor.composing) && (!state.isUpdatingSelection || androidInputManager?.isFlushing())) {
      ...
      if (anchorNodeSelectable && focusNodeInEditor) {
        ...
        range2 && (!editor.composing && !androidInputManager?.hasPendingChanges() && !androidInputManager?.isFlushing() ? editor.select(range2) : androidInputManager?.handleUserSelect(range2));
      }
  ...
  return useFlushDeferredSelectorsOnRender(), /* @__PURE__ */ jsx(ReadOnlyContext.Provider, { value: readOnly, children: /* @__PURE__ */ jsx(DecorateContext.Provider, { value: decorateContext, children: /* @__PURE__ */ jsx(RestoreDOM, { node: ref, receivedUserInput, children: /* @__PURE__ */ jsx(
    "div",
    {
...
function RenderText(props) {
  const $ = c(10);
  if (useContext(ParentContainerContext)) {
        ...
        if (editor.snapshot.context.selection) {
          ...
          for (const [point2, key] of rangePoints(selection)) {
            ...
            if (selection != null && result != null)
              ...
              let preferNext = !1;
              prev && next && (isSiblingPath(prev[1], path2) ? preferNext = !1 : pathEquals(next[1], path2) ? preferNext = !0 : preferNext = commonPath(prev[1], path2).length < commonPath(next[1], path2).length), prev && !preferNext ? selection[key] = {
                path: prev[1],
...
const addAnnotationOnCollapsedSelection = defineBehavior({
  ...
  guard: ({
    snapshot,
    event
  }) => event.originEvent.key === "ArrowDown" && isAtContainerDeadEnd(snapshot, "end"),
  actions: []
  ...
  guard: ({
    snapshot,
    event
  }) => event.originEvent.key === "ArrowUp" && isAtContainerDeadEnd(snapshot, "start"),
  actions: []
});
...
function resolveFieldOn(fields, type, fieldName) {
  const field = fields.find((candidate) => candidate.name === fieldName);
  if (!(!field || !isChildArrayField(field))) {
    if (containsOnlyPrimitiveTypes(field)) {
      console.warn(`Field '${field.name}' on '${type}' doesn't contain block or container types and will be excluded`);
      return;
    }
    return field;
  }
...
function resolveContainerField(schema, type, fieldName, parentOf) {
  if (type === "block")
    return fieldName !== "children" ? void 0 : synthesizeBlockChildrenField(schema);
  if (parentOf) {
...
function resolveNestedContainer(schema, container, chain = [], onFailure = "warn", parentOf) {
  const field = resolveContainerField(schema, container.type, container.arrayField, parentOf);
  if (!field) {
    const location = chain.length > 0 ? ` (nested inside ${chain.map((type) => `"${type}"`).join(" inside ")})` : "", message = `registerNode: field "${container.arrayField}" not found on type "${container.type}"${location}. Registration skipped.`;
    if (onFailure === "throw")
