...
function isApplicationType(value) {
 ...
 */ function detectApplicationType(projectDir) {
 ...
 */ export function parseWorkbenchCliConfig(cliConfig, projectDir) {
    ...
    if (explicit !== undefined && !isApplicationType(explicit)) {
        throw new Error(`Invalid \`applicationType\` "${explicit}" in \`unstable_defineApp\` — expected one of: ${APPLICATION_TYPES.join(', ')}`);
    }
